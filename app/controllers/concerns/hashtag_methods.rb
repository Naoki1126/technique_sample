module HashtagMethods
    ...
    def creating_structures(posts: "",post_hashtags: "",hashtags: "")
        ...
        #このメソッドはPostのActiveRecordオブジェクトをハッシュに変換し、更に一つ一つのオブジェクトに対し、idに紐づくハッシュタグを配列として格納するメソッドです。
        array = [] #最終戻り値用
        posts.each do |post|
            ...
            post_hash["hashtags"] = hashtag #投稿一つ一つのデータに['hashtag']のkeyを追加、そこにハッシュタグのデータを格納する
    ...
    def delete_records_related_to_hashtag(post_id)
        ...
        if relationship_records #中間テーブルにレコードが保存されていれば
